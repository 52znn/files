...
while true; do
    ...
    case "$choice" in
        ...
        2) SERVER_TYPE="v2ray"; break ;;
        3) SERVER_TYPE="trojan"; break ;;
        4) SERVER_TYPE="hysteria"; break ;;
        *) echo "输入无效，请输入 1-4。" ;;
    esac
done

# ====== 输入可留空的选项 ======
read -p "请输入 node_id（回车不作修改）: " NODE_ID
read -p "请输入 webapi_url（回车不作修改）: " WEBAPI_URL
read -p "请输入 webapi_key（回车不作修改）: " WEBAPI_KEY
read -p "请输入 cert_mode（回车默认 http）: " CERT_MODE
[ -z "$CERT_MODE" ] && CERT_MODE="http"
read -p "请输入 cert_domain（回车不作修改）: " CERT_DOMAIN
read -p "请输入 default_dns（回车不作修改）: " DEFAULT_DNS
...
[ -n "$WEBAPI_URL" ] && echo "webapi_url=$WEBAPI_URL"
[ -n "$WEBAPI_KEY" ] && echo "webapi_key=$WEBAPI_KEY"
echo "cert_mode=$CERT_MODE"
[ -n "$CERT_DOMAIN" ] && echo "cert_domain=$CERT_DOMAIN"
[ -n "$DEFAULT_DNS" ] && echo "default_dns=$DEFAULT_DNS"
